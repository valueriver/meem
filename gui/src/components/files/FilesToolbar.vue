...
function openInTerminal() {
    ...
    termSocket.send('data.input', { terminalId: term.activeTerminalId, input: `cd "${escaped}"\r` });
...
<template>
    <div class="shrink-0 bg-bg">
        <!-- Row 1: actions -->
        <div class="h-11 border-b border-line flex items-center gap-0.5 px-2 overflow-x-auto">
        ...
        <!-- 搜索框 -->
        <div v-if="showSearch" class="flex items-center gap-2 px-3 py-2">
        ...
        <!-- 面包屑 -->
        <div class="flex flex-wrap items-center gap-x-0.5 gap-y-1 px-3 py-2">
...
.tb-btn.is-active { color: var(--color-accent); background: var(--color-bg-hi); }
