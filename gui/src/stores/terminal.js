...
import * as termSocket from '@/utils/termSocket';
...
export const useTerminalStore = defineStore('terminal', () => {
    ...
    function sendInputRaw(input) {
        ...
        termSocket.send('data.input', { terminalId, input });
    }
    ...
    function sendResize(terminalId = activeTerminalId.value) {
        ...
        if (!terminal) return;
        termSocket.send('system.resize', { terminalId, cols: terminal.cols, rows: terminal.rows });
    }
    ...
    function ensureHandlers() {
        ...
        termSocket.on('terminal.list', (msg) => {
            const list = msg.data?.terminals || [];
            setTerminalTabs(list, msg.data?.activeTerminalId || '');
            if (list.length === 0) {
                // 后端没有存活终端 → 自动拉一个,避免前端卡在"等待终端列表..."
                termSocket.send('terminal.create', {});
            }
            setTimeout(fitActiveTerminal, 30);
        });

        termSocket.on('terminal.created', (msg) => {
        ...
        termSocket.on('terminal.closed', (msg) => {
            setTerminalTabs(
    ...
    function restartPty() {
        ...
        termSocket.send('system.command', { command: 'restart', terminalId: activeTerminalId.value });
        toast.show('已发送重启指令');
    }
