<template>
  <div class="h-full overflow-y-auto cc-thin-scroll px-6 py-5 space-y-4">
    <div>
      ...
      <div class="text-[11.5px]" style="color:#6b5a46">Claude Code 全局指令 · <span class="cc-mono">{{ data?.dir || '~/.claude' }}</span></div>
    </div>
    ...
    <div v-else-if="!data.files?.length" class="rounded-xl text-center py-12" style="border:1px dashed rgba(140,100,60,0.25);background:rgba(255,255,255,0.4)">
      <div class="text-[14px] font-bold mb-1">还没写 CLAUDE.md</div>
      <div class="text-[11.5px] max-w-md mx-auto" style="color:#6b5a46">Claude Code 每次启动都会加载 —— 适合放个人偏好 / 约定 / 硬规则。</div>
      <button class="mt-4 cc-btn-primary rounded-lg px-4 py-2 text-[12.5px] font-semibold disabled:opacity-50"
        :disabled="creating" @click="createFile">{{ creating ? '创建中...' : '+ 新建 CLAUDE.md' }}</button>
    </div>
...
在这里写 Claude Code 全局约束 / 个人偏好,每次启动都会被加载。
`;
